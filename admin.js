/**
 * admin.js
 * Painel administrativo — lê dados DO SUPABASE (não localStorage)
 * Funciona em qualquer dispositivo/navegador autenticado.
 */

const Admin = (() => {

  let _isLoggedIn = false;
  // Usa localStorage para persistir sessão entre abas/recargas
  // (não depende de sessionStorage que morre ao fechar)
  ...
  let _clientMonthKey = null;

  // ── Auth ──────────────────────────────────────────────────

  function isLoggedIn() {
    if (_isLoggedIn) return true;
    // Verifica localStorage E sessionStorage para robustez cross-device
  ...
  async function enter() {
    ...
    _clientMonthKey = Storage.currentMonthKey();
    const [y, m] = _clientMonthKey.split('-');
    ...
    // Mostra o ID do cliente na strip
    const cfg = Storage.getConfig();
    const clientUserId = cfg.clientUserId || 'gisele';
    const clientIdLabel = document.getElementById('adm-client-id-label');
    if (clientIdLabel) clientIdLabel.textContent = clientUserId;

    // Mostra estado de carregamento
    _setLoadingState(true);

    // Carrega dados do cliente do Supabase
    await _loadClientDataFromSupabase();

    _loadChartJs().then(() => {
      if (_clientData) {
        _renderAdminData(_clientData);
      } else {
        // Fallback: usa localStorage se Supabase não responder
        _renderAdminData(Storage.getMonthData(_clientMonthKey));
      }
    });
  ...
  function _setLoadingState(loading) {
    const loadingEl = document.getElementById('adm-loading-banner');
    if (loadingEl) loadingEl.hidden = !loading;
  }

  // ── Carregar dados do cliente DO SUPABASE ─────────────────

  async function _loadClientDataFromSupabase() {
    ...
    try {
      const result = await SupabaseSync.pullMonthData(_clientMonthKey, clientUserId);
      if (result.ok && result.data) {
        _clientData = { lancamentos: [], ...result.data };
      } else {
        _clientData = null;
      }
    } catch {
  ...
  function _renderAdminData(data) {
    ...
    // Atualiza info de sincronização
    const syncInfoEl = document.getElementById('adm-sync-info');
    if (syncInfoEl) {
      const source = (d === _clientData) ? '☁️ Supabase' : '💾 Local';
      syncInfoEl.textContent = `Fonte: ${source}`;
    }
  }
  ...
  function _loadFeed() {
    ...
    SupabaseSync.fetchActivity(Storage.currentMonthKey(), 50, clientUserId).then(res => {
      const listEl = document.getElementById('feed-list');
      if (listEl) listEl.innerHTML = '';
      if (!res.ok || !res.data.length) {
        // Fallback para feed local
        const local = Storage.getFeed();
        local.forEach(entry => _appendFeedItem(entry, false));
        return;
      }
  ...
  function _startRealtime() {
    ...
    SupabaseSync.setCallbacks({
      onMonthChange: (record) => {
        if (record.data) {
          _clientData = { lancamentos: [], ...record.data };
          if (window.Chart) {
            _renderAdminData(_clientData);
          } else {
            _loadChartJs().then(() => _renderAdminData(_clientData));
          }
          UI.toast('Dados da cliente atualizados em tempo real ☁️', 'info', 2500);
        }
      },
      onActivity: (record) => {
    ...
    SupabaseSync.connectRealtime((status) => _setRealtimeStatus(status), clientUserId);
  }
  ...
  async function refreshClientData() {
    ...
    if (_clientData) {
      ...
      UI.toast('Dados atualizados do Supabase ✓', 'success');
    } else {
      UI.toast('Supabase offline — mostrando dados locais', 'warn');
      _renderAdminData(Storage.getMonthData(_clientMonthKey));
    }
  }
  ...
  async function sendMessage() {
    ...
    if (SupabaseSync.isConfigured()) {
      const result = await SupabaseSync.pushMessage(text, type, clientUserId);
      UI.toast(result.ok ? 'Mensagem enviada para a cliente! ✓' : 'Erro ao enviar — verifique o Supabase', result.ok ? 'success' : 'warn');
    } else {
      // Fallback local (só funciona no mesmo dispositivo)
      Storage.saveMessage({ text, type, ts: new Date().toISOString() });
      UI.toast('Mensagem salva localmente (Supabase não configurado)', 'warn');
    }
  ...
  function _maybePushNotification(entry) {
    ...
    new Notification('Caixinhas — Atividade da cliente', {
  ...
  async function checkIncomingMessages() {
    if (SupabaseSync.isConfigured()) {
      const cfg = Storage.getConfig();
      const clientUserId = cfg.clientUserId || 'gisele';
      const res = await SupabaseSync.fetchUnreadMessages(clientUserId);
      if (res.ok && res.data.length) {
    ...
    if (local) { _showAlertBanner(local.text, local.type || 'info'); }
  }
